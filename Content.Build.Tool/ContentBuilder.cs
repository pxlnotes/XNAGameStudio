...
namespace Content.Build
{
    ...
    {
        public static void UpdatePipelineAssemblies(string[] refreshedPipelineAssemblies) 
        {
            pipelineAssemblies = null;
            pipelineAssemblies = refreshedPipelineAssemblies;
        }
        ...
        {
            get { return Path.Combine(buildDirectory, ""); }
        }
        ...
        /// </summary>
        public ContentBuilder(BuildProperties buildProperties)
        {
            CreateTempDirectory();
            CreateBuildProject(buildProperties);
        }
        ...
        /// </summary>
        void CreateBuildProject(BuildProperties buildProperties)
        {
            ...
            buildProject.SetProperty("Clean", "true");

            buildProperties.ApplyTo(buildProject);
        ...
        /// </summary>
        public void Add(string filename, string name, string importer, string processor, ContentCompileItem contentCompileItem = null)
        {
            string relativePath = GetPathFromContent(filename);
            ...
            { 
                foreach (var metadata in contentCompileItem.ExtraMetadata) 
                {
                    //example:
                    //item.SetMetadataValue("ProcessorParameters_GenerateTangentFrames", "True");

                    item.SetMetadataValue(metadata.Key, metadata.Value);                                        
                }
            }
        ...
        static string GetPathFromContent(string fullPath)
        {
            string separator = Path.DirectorySeparatorChar.ToString();
            string marker = separator + "Content" + separator;

            int index = fullPath.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
            if (index < 0)
                throw new InvalidOperationException("Path does not contain a Content folder.");
