...
namespace Content.Build
{
    ...
    internal class ErrorLogger : ILogger
    {
        ...
        public static void WriteErrorFile(string baseDirectory, string errorMessage, Exception ex = null)
        {
            string logDirectory = Path.Combine(baseDirectory, "Logs");
            Directory.CreateDirectory(logDirectory);

            string timestamp = DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss");
            string filePath = Path.Combine(logDirectory, $"error_{timestamp}.log");
