...
) : COLOR0
{
    ...
    // --- Safe normal unpack ---
    float3 nVec = normal.xyz * 2.0 - 1.0;
    float nLen = length(nVec);
    float3 n = (nLen > 1e-4) ? nVec / nLen : float3(0, 0, 1);
    ...
    float3 h = (hLen > 1e-4) ? hVec / hLen : n;
    ...
    // --- Specular (FXC-safe pow) ---
    float specPower = specular.w * 255.0;
    float safeNdoth = max(ndoth, 1e-4);
    specular.xyz *= LightColor * pow(safeNdoth, specPower);

    // --- Reflection ---
    reflect *= 1.0 - normal.w;

    // --- Glow ---
    float glow_intensity =
        saturate(dot(glow.xyz, 1.0) + dot(specular.xyz, 1.0));
