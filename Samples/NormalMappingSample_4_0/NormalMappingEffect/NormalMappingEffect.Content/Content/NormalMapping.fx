...
float4 PixelShaderFunction(VS_OUTPUT input) : COLOR0
{
    // Normal map decode (CRITICAL)
    float3 normalFromMap = tex2D(NormalMapSampler, input.texCoord).xyz * 2.0f - 1.0f;
    ...
    float safeSpecPower = max(SpecularPower, 1e-4);
    float spec = pow(rDotV, safeSpecPower);

    float4 specular = Shininess * LightColor * spec;
