...
namespace MultipassLightingSample
{
    ...
    public class MultipassLighting : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            graphics.GraphicsDevice.BlendState = new BlendState
            {
        ...
        protected void DrawLights()
        {
            ...
            // XNA 4.0 / MonoGame: bind buffers directly (no Vertices[], no VertexDeclaration)
            graphics.GraphicsDevice.SetVertexBuffer(meshPart.VertexBuffer);
            graphics.GraphicsDevice.Indices = meshPart.IndexBuffer;

            // loop through the lights and draw a flat-colored sphere
            foreach (var pass in pointLightMeshEffect.CurrentTechnique.Passes)
            {
                for (int i = 0; i < numLights; i++)
                {
                    lightMeshWorld.M41 = lights[i].Position.X;
                    ...
                    pointLightMeshEffect.Parameters["world"].SetValue(lightMeshWorld);
                    pointLightMeshEffect.Parameters["lightColor"]
                        .SetValue(lights[i].Color.ToVector4());
                    ...
                    graphics.GraphicsDevice.DrawIndexedPrimitives(
                        ...
                        0,                    // minVertexIndex
                        meshPart.NumVertices, // numVertices
