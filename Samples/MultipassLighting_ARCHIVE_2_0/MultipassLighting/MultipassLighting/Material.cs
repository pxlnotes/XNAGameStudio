...
namespace MultipassLightingSample
{
    public class Material
    {
            ...
            Effect baseEffect)
        {
            ...
            effectInstance = baseEffect.Clone();
        ...
        public void BeginBatch(Model model, ref Matrix world)
        {
            ...
            // sample assumes a single mesh / mesh part
            currentMesh = model.Meshes[0];
            currentMeshPart = currentMesh.MeshParts[0];

            // XNA 4.0 / MonoGame: bind buffers directly
            device.SetVertexBuffer(currentMeshPart.VertexBuffer);
            device.Indices = currentMeshPart.IndexBuffer;

            // set per-instance parameter
            effectInstance.Parameters["world"].SetValue(world);

            //////////////////////////////////////////////////////////////
            // Example 2.1: State Batching (MonoGame version)
            //////////////////////////////////////////////////////////////

            // --- Ambient pass (opaque, writes depth) ---
            device.BlendState = BlendState.Opaque;
            device.DepthStencilState = DepthStencilState.Default;
            device.RasterizerState = RasterizerState.CullCounterClockwise;

            var ambientPass = effectInstance.CurrentTechnique.Passes["Ambient"];
            ambientPass.Apply();

            device.DrawIndexedPrimitives(
                PrimitiveType.TriangleList,
                0,                          // baseVertex (removed in MG)
                0,                          // minVertexIndex
                currentMeshPart.NumVertices,
                currentMeshPart.StartIndex,
                currentMeshPart.PrimitiveCount
            );

            // --- Prepare for point-light passes (additive, no depth writes) ---
            ...
            // PointLight pass will be applied per-light later
            // (equivalent to leaving the pass "begun" in old XNA)
        }
        ...
        public void DrawModel()
        {
            ...
            // XNA 4.0 / MonoGame:
            // CommitChanges() no longer exists – parameter changes are applied via Apply()
            var pass = effectInstance.CurrentTechnique.Passes["PointLight"];
            pass.Apply();
        ...
        public void EndBatch()
        {
            // No Effect.End / Pass.End in XNA 4.0 / MonoGame
            currentMeshPart = null;
            currentMesh = null;
        }
        #endregion
