...
) : COLOR0
{
    ...
    outCol = tex2D(nebula1, tex0) * (0.2 + layerFactor * 0.8);
    //outCol += tex2D(nebula2, tex1) * (1.0 - saturate(layerFactor * 0.8));
    outCol += tex2D(nebula2, tex1) * (0.2 + 0.8 * (1.0 - layerFactor));
    outCol += tex2D(stars, tex2);
...
technique RenderBackdrop
{
	pass P0
	{
		...
		VertexShader = compile vs_3_0 backdropVS();
