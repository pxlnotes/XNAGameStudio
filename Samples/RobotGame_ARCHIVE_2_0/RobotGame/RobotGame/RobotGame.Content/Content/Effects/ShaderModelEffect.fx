...
VS_OUTPUT _VertexShader( VS_INPUT input )
{
    VS_OUTPUT output;
...
float4 _PixelShader( VS_OUTPUT input ) : COLOR0
{    
    ...
    float ndv = saturate(dotView);
    float3 specular = specularColor.rgb * pow(ndv, specularColor.a);
    ...
    float3 ambient = AmbientLightColor.rgb * environmentColor.rgb;
        
    // return the combined result.
    //    return float4( (diffuse.rgb + AmbientLightColor.rgb) * diffuseTexture.rgb + 
    //                specular.rgb, diffuseTexture.a);
    return float4((diffuse.rgb + ambient) * diffuseTexture.rgb + specular.rgb, diffuseTexture.a);
}
