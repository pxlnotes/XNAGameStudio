...
namespace RobotGame
{
    ...
    public abstract class BaseStageScreen : GameScreen
    {
        ...
        protected RenderTarget2D resolveTexture = null;
        ...
        public void RestoreRenderTarget()
        {
            ...
            //                                pp.BackBufferHeight / 4,
            //                                1, format);
            // Full-resolution render target (replaces ResolveTexture2D)
            resolveTexture = new RenderTarget2D(
                GraphicsDevice,
                pp.BackBufferWidth,
                pp.BackBufferHeight,
                false,
        ...
        internal void OnPostDraw3DScreen(object sender, EventArgs e)
        {
            ...
            // Backbuffer is already resolved in XNA 4.0
            // resolveTexture is now a RenderTarget2D holding the scene
            ...
            // Disable depth buffer (XNA 4.0 way)
            device.DepthStencilState = DepthStencilState.None;

            ////////////// BoosterEffect (Radial Blur)
            for (int index = 0; index < GameLevel.PlayerCountInLevel; index++)
            {
                GamePlayer player = GameLevel.GetPlayerInLevel(index);
                Viewport currentViewport =
                ...
                {
                    float xCenter =
                        0.5f * ((float)currentViewport.Width / defaultViewport.Width) +
                        ((float)currentViewport.X / defaultViewport.Width);

                    float yCenter =
                        0.5f * ((float)currentViewport.Height / defaultViewport.Height) +
                        ((float)currentViewport.Y / defaultViewport.Height);
                ...
                // Restore depth (matches original intent)
                                  ...
                                  Effect effect, int pass)
        {
            ...
            DrawPostScreenEffect(rtTexture, effect, pass,
    ...
    int pass)
        {
            ...
            // Back to backbuffer (implicit resolve)
            device.SetRenderTarget(null);

            // Bind secondary texture slot (slot 1)
            device.Textures[1] = baseTexture;

            // SpriteBatch state equivalent to SpriteBlendMode.None + Immediate
            sprite.Begin(
                SpriteSortMode.Immediate,
                BlendState.Opaque,
                SamplerState.LinearClamp,
            ...
            // Draw full-screen quad
            sprite.Draw(
                renderTarget, // RenderTarget2D is the resolved texture in XNA 4.0
                new Rectangle(0, 0, defaultViewport.Width, defaultViewport.Height),
                new Rectangle(0, 0, renderTarget.Width, renderTarget.Height),
                Color.White
            );
